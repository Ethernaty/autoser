...
class AuthService:
    ...
    def register(self, *, email: str, password: str, tenant_name: str, tenant_slug: str | None) -> AuthResult:
        ...
        with SqlAlchemyUnitOfWork(session_factory=SessionLocal) as uow:
            ...
            self._bootstrap_subscription(uow.session, tenant_id=tenant.id)
    ...
    def _bootstrap_subscription(self, db: Session, *, tenant_id: UUID) -> None:
        from app.core.config import get_settings

        cfg = get_settings()
        plan_repo = PlanRepository(db)
        plan = plan_repo.get_by_name(cfg.default_plan_name)
        if plan is None:
            active_plans = plan_repo.list_active()
            if not active_plans:
                raise AppError(status_code=503, code="plan_not_found", message="No active plans configured")
            plan = active_plans[0]

        now = datetime.now(UTC)
        trial_end = now + timedelta(days=cfg.default_trial_days) if cfg.default_trial_days > 0 else None

        subscription_repo = SubscriptionRepository(db=db, tenant_id=tenant_id)
        ...
        event_repo = BillingEventRepository(db=db, tenant_id=tenant_id)
